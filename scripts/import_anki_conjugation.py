...
def write_chunk(path: str, rows: list[dict], offset: int, total: int, source: str) -> None:
    with open(path, 'w', encoding='utf-8', newline='\n') as f:
        ...
        f.write(f'-- Cards in this chunk: {len(rows)} (sequences {offset + 1}..{offset + len(rows)} of {total})\n\n')
...
def main() -> int:
    ...
    for idx in range(num_chunks):
        ...
        write_chunk(out_path, rows[start:end], start, total, source)
        print(f'Wrote {end - start:4d} cards -> {out_path}')

    print(f'\nDone: {total} cards in {num_chunks} file(s) under {args.out_dir}')
    return 0
